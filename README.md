...
Repository initialization, verified local chunk storage, snapshot manifest serialization, filesystem scanning, and the first local backup flow are implemented. The remaining commands are placeholders while the first local backup vertical slice is completed.
